...
function ars_add_admin_menu() {
    add_menu_page( 'Search Relevance', 'Search Relevance', 'manage_options', 'ars-settings', 'ars_render_settings_page', 'dashicons-search', 65 );
    add_submenu_page( 'ars-settings', 'Settings', 'Settings', 'manage_options', 'ars-settings', 'ars_render_settings_page' );
    add_submenu_page( 'ars-settings', 'Pinned Resultaten', 'Pinned Resultaten', 'manage_options', 'ars-pinned', 'ars_render_pinned_page' );
    add_submenu_page( 'ars-settings', 'Logging', 'Logging', 'manage_options', 'ars-logging', 'ars_render_logging_page' );
}
...
function ars_render_pinned_page() {
    global $wpdb;
    ...
    ?>
    <div class="wrap">
        <h1>Pinned Zoekresultaten (<?php echo strtoupper($current_lang); ?>)</h1>
...
function ars_render_logging_page() {
    global $wpdb; $table_logs = $wpdb->prefix . 'ars_logs';
    $current_lang = defined('ICL_LANGUAGE_CODE') ? ICL_LANGUAGE_CODE : 'all';
    $options = get_option( 'ars_settings', array() );
    $days = isset($options['log_retention']) ? $options['log_retention'] : 30;

    $top  = $wpdb->get_results($wpdb->prepare("SELECT search_term, COUNT(*) as c FROM $table_logs WHERE lang = %s GROUP BY search_term ORDER BY c DESC LIMIT 20", $current_lang));
    $zero = $wpdb->get_results($wpdb->prepare("SELECT search_term, COUNT(*) as c FROM $table_logs WHERE lang = %s AND result_count = 0 AND (result_ids IS NULL OR result_ids = '') GROUP BY search_term ORDER BY c DESC LIMIT 20", $current_lang));
    ?>
    <div class="wrap">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top:20px;">
            <div class="card" style="padding: 20px; background:#fff; border:1px solid #ccd0d4;">
                <h3>Populairste zoekopdrachten</h3>
                <table class="widefat striped">
                    <thead><tr><th>Term</th><th>Aantal</th></tr></thead>
                    <tbody>
                        <?php foreach($top as $t) echo "<tr><td>".esc_html($t->search_term)."</td><td>".(int)$t->c."</td></tr>"; ?>
                    </tbody>
                </table>
            </div>

            <div class="card" style="padding: 20px; background:#fff; border:1px solid #ccd0d4;">
                <h3>Zoekopdrachten zonder resultaat</h3>
                <table class="widefat striped">
                    <thead><tr><th>Term</th><th>Aantal</th></tr></thead>
                    <tbody>
                        <?php foreach($zero as $t) echo "<tr><td>".esc_html($t->search_term)."</td><td>".(int)$t->c."</td></tr>"; ?>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    jQuery(document).ready(function($) {
        $('#ars-wipe-logs').on('click', function() {
            if(confirm('Weet je zeker dat je ALLE logs wilt wissen?')) {
                $.post(ajaxurl, { action: 'ars_clear_logs', nonce: '<?php echo wp_create_nonce("ars_log_nonce"); ?>' }, function() { location.reload(); });
            }
        });
        $('#ars-prune-logs').on('click', function() {
            if(confirm('Oude logs wissen?')) {
                $.post(ajaxurl, { action: 'ars_prune_logs', nonce: '<?php echo wp_create_nonce("ars_log_nonce"); ?>' }, function() { location.reload(); });
            }
        });
    });
    </script>
